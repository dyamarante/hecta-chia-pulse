...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'shadow-md bg-[#F5F0E9]/90 backdrop-blur-sm' : 'bg-[#F5F0E9]'}`}>
      ...
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          {navItems.map((item) => (
            <a 
              key={item.name} 
              href={item.href} 
              className="text-hecta-gray hover:text-hecta-gold transition-colors duration-200 font-medium"
            >
              {item.name}
            </a>
          ))}
        ...
        <button 
          ...
          className="md:hidden text-hecta-gray hover:text-hecta-gold"
      ...
      {/* Mobile Navigation */}
      {isMenuOpen && (
        <div className="md:hidden bg-[#F5F0E9] shadow-lg">
            ...
            <nav className="flex flex-col space-y-4">
              {navItems.map((item) => (
                <a 
                  key={item.name} 
                  href={item.href} 
                  className="text-hecta-gray hover:text-hecta-gold py-2 transition-colors duration-200 font-medium"
